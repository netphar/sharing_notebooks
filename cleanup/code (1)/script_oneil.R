...
single = read.xlsx("156849_1_supp_0_w2lh45.xlsx")
combo = read.xlsx("156849_1_supp_1_w2lrww.xlsx")
names(combo)[2] = "cell_line_name"
names(combo)[3] = "drug_row"
names(combo)[4] = "conc_r"
names(combo)[5] = "drug_col"
names(combo)[6] = "conc_c"
names(combo)

combo$block_id = as.numeric(as.factor(paste(combo$cell_line_name,combo$drug_row,combo$drug_col)))
combo$response = apply(combo[,c(8:11)],1,function(x) mean(x,na.rm=T))
response = combo[, c(2,3,4,5,6,14,15)]

new_list = list()
for (i in unique(combo$block_id)){
  cat('\r',i)
  drug_row = unique(combo$drug_row[which(combo$block_id==i)])
  drug_col = unique(combo$drug_col[which(combo$block_id==i)])
  cell = unique(combo$cell_line_name[which(combo$block_id==i)])
  conc_r = unique(combo$conc_r[which(combo$block_id==i)])
  conc_c = unique(combo$conc_c[which(combo$block_id==i)])
  
  # fit single drugs
  index = which(single$drug_name==drug_row & single$cell_line==cell)
  data_row = single[index,]
  inhibition = 1-as.numeric(c(data_row$viability1, data_row$viability2, data_row$viability3, data_row$viability4, data_row$viability5, data_row$viability6))
  conc = as.numeric(rep(data_row$`Drug_concentration.(µM)`,6))
  tmp = data.frame(inhibition = inhibition, conc = conc)
  
  res_row <- tryCatch({
    drm(inhibition~conc, data=tmp, fct=LL.4())
    #drm(effect ~ conc, data = drug.col, fct = LL.4(fixed = fixed), 
    #    na.action = na.omit, control = drmc(errorm = FALSE, noMessage = T))
  },error = function(e) {
    drm(inhibition~log(conc), data=tmp, fct=L.4())
    #drm(effect ~ log(conc), data = drug.col, fct = L.4(fixed = fixed), 
    #    na.action = na.omit, control = drmc(errorm = FALSE, noMessage = T))
  })
  
  if(length(grep("LL.4", res_row$call$fct))>0) predict = predict(res_row, data.frame(conc = conc_r)) else predict = predict(res_row, data.frame(conc = log(conc_r)))
  
  new_row = data.frame(cell_line_name = cell, drug_row = drug_row, conc_r=conc_r, drug_col = drug_col, 
                       conc_c = 0, block_id = i, response = predict)
  
  
  index = which(single$drug_name==drug_col & single$cell_line==cell)
  data_col = single[index,]
  inhibition = 1-as.numeric(c(data_col$viability1, data_col$viability2, data_col$viability3, data_col$viability4, data_col$viability5, data_col$viability6))
  conc = as.numeric(rep(data_col$`Drug_concentration.(µM)`,6))
  tmp = data.frame(inhibition = inhibition, conc = conc)
  ...
  if(length(grep("LL.4", res_col$call$fct))>0) predict = predict(res_col, data.frame(conc = conc_c)) else predict = predict(res_col, data.frame(conc = log(conc_c)))
  
  new_col = data.frame(cell_line_name = cell, drug_row = drug_row, conc_r=0, drug_col = drug_col, 
                       conc_c = conc_c, block_id = i, response = predict)
  new_list[[i]] = rbind(new_row, new_col)
}
new_list = do.call(rbind, new_list)


# READ RESPONSE DATA
response2 = read.xlsx("C:/Users/Localadmin_jtang/Dropbox/drugcomb/data/Oneil/response_oneil_raw.xlsx")
tmp = merge(new_list, response2, by = c("cell_line_name","drug_row","drug_col","conc_r","conc_c"))
