...
length(which(index_duplidated == T)) == 0
...
write.csv(surface_all[c(3000001:nrow(surface_all)), -which(names(surface_all) %in% c("drug_row", "drug_col", "conc_r_unit","conc_c_unit"))], "C:\\Users\\Localadmin_jtang\\Dropbox\\drugcomb\\data\\database\\surface_part3.csv", row.names = F)


write.table(surface_all[c(1:1500000), -which(names(surface_all) %in% c("drug_row", "drug_col", "conc_r_unit", "conc_c_unit", "drug_row_id", "drug_col_id"))], "C:\\Users\\Localadmin_jtang\\Dropbox\\drugcomb\\data\\database\\surface_part1.txt", row.names = F, sep = "\t")
write.table(surface_all[c(1500001:3000000), -which(names(surface_all) %in% c("drug_row", "drug_col", "conc_r_unit","conc_c_unit","drug_row_id", "drug_col_id"))], "C:\\Users\\Localadmin_jtang\\Dropbox\\drugcomb\\data\\database\\surface_part2.txt", row.names = F, sep = "\t")
write.table(surface_all[c(3000001:nrow(surface_all)), -which(names(surface_all) %in% c("drug_row", "drug_col", "conc_r_unit","conc_c_unit"))], "C:\\Users\\Localadmin_jtang\\Dropbox\\drugcomb\\data\\database\\surface_part3.txt", row.names = F, sep = "\t")
