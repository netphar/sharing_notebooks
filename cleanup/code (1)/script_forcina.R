...
library(drc)
library(nleqslv)
setwd("C:\\Users\\Localadmin_jtang\\Dropbox\\drugcomb\\data\\Forcina\\Shuyu")
rm(list=ls(all=TRUE))
...
# drug_database = read.xlsx("C:\\Users\\Localadmin_jtang\\Dropbox\\drugcomb\\data\\database\\drug.xlsx")
response_forcina = read.csv("response_template.csv", stringsAsFactors = F)
# response_forcina$drug_col[which(response_forcina$drug_col_id == 31)] = drug_database$drug_name[which(drug_database$drug_id == 31)]  # change TMZ into TEMOZOLOMIDE 
...
response_forcina = ddply(response_forcina,c("cell_line_name","drug_row","drug_col","block_id"), transform, row = own_rank(conc_r), col = own_rank(conc_c))

scores = list()
m = unique(response_forcina$block_id)
options(show.error.messages = F)

for(i in m){
  cat('\r',i)
  index = which(response_forcina$block_id == i)
  data.tmp = response_forcina[index,]
...
response_forcina_with_scores = do.call(rbind, scores)
response_forcina_with_scores$block_id = response_forcina_with_scores$block_id 
write.csv(response_forcina_with_scores,"C:/Users/Localadmin_jtang/Dropbox/drugcomb/data/forcina/response_forcina.csv", row.names = F)
save.image("C:/Users/Localadmin_jtang/Dropbox/drugcomb/data/forcina/forcina_part1.RData")
