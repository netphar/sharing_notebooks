...
---
###TODO 
* why the same function fails at ZIP 
...
```
I know that 104 combo throws error when calculating Loewe synergy score, because i do ```{r}
debugonce(Loewe)
...
```{r}
ZIP <- function (response.mat,correction, Emin, Emax ,nan.handle = c("LL4", "L4")) {
# scores <- list()
#  method <- "ZIP"
  out <- tryCatch(ZIP_modded(response.mat,correction, Emin,Emax, nan.handle), error = function(e) NA)
#  {
#  x$method <- method
#  x$scores <- x$dose.response.mats
#  x$scores[!is.na(x$scores)] <- NA
...
files were saved using sink(), print(with max option set), sink()
