...
ZIP2 = function (response.mat, correction = TRUE, Emin = 0, Emax = NA, 
                 nan.handle = c("L4")) 
{
  if (correction) {
    nan.handle <- match.arg(nan.handle)
    response.mat <- BaselineCorrectionSD2(response.mat, NA, NA, nan.handle)$corrected.mat
  }
  single.fitted <- FittingSingleDrug2(response.mat, fixed = c(NA, Emin, Emax, NA), nan.handle)
  ...
  for (i in 2:ncol(response.mat)) {
    ...
    if(nrow(tmp) == 1){
      ...
      tmp.model = tryCatch(
        {
          tmp.model <- drm(inhibition ~ dose, data = tmp, fct = LL.4(fixed = c(NA, tmp.min, Emax, NA)), na.action = na.omit, control = drmc(errorm = FALSE, noMessage = T))
        }, 
        warning = function(w) {
  ...
  for (i in 2:nrow(updated.single.mat)) {
    for (j in 2:ncol((updated.single.mat))) {
      zip.mat[i, j] <- updated.single.mat[i, 1] + updated.single.mat[1, j] - updated.single.mat[i, 1] * updated.single.mat[1, 
                                                                                                                           j]/100
    }
  }
  
  # Bulat used the following 3 lines, but I did not
  # fitted.mat[1, 1] <- 0
  # zip.mat[1, 1] <- 0
...
ZIP <- function (response.mat,correction = T, Emin = 0, Emax = NA,nan.handle = c("L4")) {
  # scores <- list()
  #  method <- "Loewe"
  out <- tryCatch(ZIP2(response.mat,correction = correction, Emin = Emin, Emax = Emax, 
...
CalculateSynergy(input, method = 'ZIP', correction = T, Emin = 0, Emax = NA) -> input.ZIP
saveRDS(object = input.ZIP, file = '0309_reshaped.ZIP')
